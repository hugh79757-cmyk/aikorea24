...
def _layer1_substring_match(
    ...
    # Trailing particles / descriptive suffixes stripped from noun chunks
    # before matching. The noun regex [가-힣]{2,8} greedily grabs
    # noun+particle combos (e.g. '행wers' = 행사+를, '준공할' = 준공+할,
    # '그룹의' = 그룹+의) that never appear verbatim in the source body,
    # producing false "출처 미매칭" blocks. Strip them so the bare noun
    # stem is matched instead (REQ-39-11).
    _suffixes = sorted({
        '입니다', '했습니다', '합니다', '됩니다', '됐습니다', '였습니다',
        '하겠습니다', '하겠습니다', '했습니다', '합니다', '됩니다',
        'afen습니다', 'afen았습니다', '있습니다', '없습니다', '했었다',
    ...
    for n in nouns:
        ...
        for ending in _verb_endings:
            if stem.endswith(ending) and len(stem) - len(ending) >= 2:
                stem = stem[:-len(ending)]
                break
        # Strip trailing particles/descriptive suffixes (longest first)
        for suf in _suffixes:
            if stem.endswith(suf) and len(stem) - len(suf) >= 2:
                stem = stem[:-len(suf)]
                break

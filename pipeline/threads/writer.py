...
1번 — 3-stanza. 각 stanza는 2~3줄. 각 줄 20~35자.
  stanza1 (통념): 줄1 통념도입 + 줄2 구체내용
  stanza2 (전환): 줄1 전환시그널 + 줄2 첫 증거(인명·숫자)
...
- 통념은 "~라는 말이 있었음", "~라는 주장이 퍼졌음", "~로 알려졌음", "~라는 인식이 지배적이었음" 등 **객관적 사실/인용**으로만 시작할 것.
...
첫 문장 (통념으로 재구성): {pitch['hook']}
  ...
  예시:
  - "AI 교육이 진보를 약속하지만 역설" → "AI 교육이 시민을 강화한다는 말, 당연시되어 왔음"
  - "오픈소스 AI가 대세라지만" → "오픈소스가 혁신을 이끈다는 주장, 업계 표준처럼 받아들여졌음"
  - "GPU 부족이 해소됐다지만" → "칩 공급난이 끝났다는 보도, 올 초부터 반복돼 왔음"

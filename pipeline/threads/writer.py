...
- 한 줄에 문장을 길게 늘어놓지 않는다. 문장이 길면 의미 단위로 끊어서 두 줄로 나눠라. 한 줄은 20자에서 25자 내외로 유지하고, 한 줄이 그보다 길어지면 무조건 줄바꿈해라.
...
1번 — 통념 세우기 (기사 밖):
  기사 밖의 일반적 믿음/기대/상황/통념을 세운다.
  기사 본문의 사실을 직접 인용하지 않는다.
  예: "지난 2년간, 다들 AI가 사람 일자리를 대체한다고 믿었음."
  예: "~라는 말, 지난 N년간 모두가 믿었음."
  예: "다들 ~라고 생각했음. 당연한 상식이었음."
  시작 패턴: "~라는 말, 지난 N년간 모두가 믿었음" / "원래 ~라는 게 통념이었음" / "다들 ~라고 생각했음" / "지금까지는 ~였음"

---
2번 — 전환 = but_line:
  1번 통념을 기사의 but_line(모순/역설 한 줄)이 깨는 구조.
  반드시 "하지만..." 전환으로 시작할 것.
  but_line을 그대로 전환점으로 사용한다.
  시작 표현 (매번 다양하게):
  - "근데 이게 깨지는 중임."
  - "하지만 여기서 방향이 꺾임."
  - "그런데 사실은 반대임."
...
=== 카드별 필수 규칙 (반드시 준수) ===
- 1번 카드: but_line에 나온 통념의 반대편을 일반화해서 통념으로 세우기. 기사 본문 인용 금지.
- 2번 카드: but_line을 전환점으로 사용. "하지만..." 흐름으로 1번 통념 깨기.
- 3번 카드: 기사에서 but_line을 뒷받침하는 구체적 숫자/발언 인용. 증거 집중.

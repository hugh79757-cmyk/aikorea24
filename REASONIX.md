...
## 수정 4: DiffusionGemma 첫 글자 드랍 대응

### 문제
DiffusionGemma가 hook 첫 글자를 반복적으로 드랍:
- `미국` → `국`, `실리콘밸리` → `리콘밸리`, `1년` → `년`

5회 재시도 모두 실패 → GPT-4o-mini fallback까지 가는 시간 낭비.

### 변경
**writer_v3.py:** `max_attempts = 5` → `max_attempts = 2`
2회 실패 시 즉시 GPT-4o-mini fallback으로 전환. GPT-4o-mini는 첫 글자 드랍 문제가 없음.

---

## 수정 5: narrative_pitcher 강제 연결 완화

### 문제
시스템 프롬프트에 `"반드시 2개 이상의 다른 기사를 연결할 것"` 이라는 강제 규칙이 있어서, 전혀 관련 없는 기사(예: YouTube 라벨링 + 미-이란 전쟁)가 억지로 클러스터링됨.

### 변경
**기존:** `- 반드시 2개 이상의 다른 기사를 연결할 것.`
**변경:** `- 2개 이상의 기사를 연결하면 더 강력함. 단, 관련 없는 기사를 억지로 연결하지 말 것.`

단일 기사만으로도 쓰레드 작성 가능 (1개여도 검증 통과).

---

---

# v3 Threads Writer — 2026-06-21 트러블슈팅

## 문제 1: DiffusionGemma 첫 글자 드랍
**증상:** `"미국 청소년"` → `"국 청소년"`, `"실리콘밸리"` → `"리콘밸리"`
**원인:** DiffusionGemma 모델 레벨 문제. 첫 토큰을 드롭하는 현상.
**해결:** `max_attempts 5→2`. 2회 실패 시 GPT-4o-mini fallback. (DiffusionGemma 복귀 후에도 동일)
**2-pass 도입:** `fix_cards()` 함수로 2차 검증. DiffusionGemma가 생성 후 다시 DiffusionGemma로 글자 단위 오류 수정.

---

## 문제 2: 연도 할루시네이션 (1차)
**증상:** `"2026년 6월 20일. 미국과 이란의 무력 충돌이 발생했다"` — 기사 본문에 없는 연도/날짜 생성
**원인:** `validate_year()`가 `pub_date`(발행일) 연도를 쓰레드에 강제. 프롬프트도 "발행일 연도 사용" 지시.
**해결:** `expected_year` 강제 검증 제거 → `"쓰레드 연도 ⊆ 본문 연도"` 로 변경. hook(1번 카드 첫 줄, pitcher 생성)은 검증 제외. 현재 연도는 본문에 없어도 허용.

---

## 문제 3: 연도 할루시네이션 (2차) — 자연스러운 연도 사용 차단
**증상:** 기사 본문에 `2026`이 없으면 쓰레드에서 `2026`을 쓰는 것도 전부 차단 → 발행률 0%
**원인:** `validate_year`가 본문 텍스트에 없는 연도를 무조건 할루시네이션으로 간주
**해결:** `body_years ∪ {current_year}`를 허용. 2026년 기사면 2026 사용 정상 허용.

---

## 문제 4: article_ids 타입 불일치 → fallback 오염
**증상:** 피치가 `['#32698']`(str + #접두사) 반환 → DB int(32698)와 매칭 실패 → `related`가 비어서 `all_articles[:2]` fallback 발동 → gokorea.kr 가짜 기사로 내용 오염
**원인:** pitcher는 str, DB는 int. `32698` in `['#32698']` → False
**해결:** `str(aid).lstrip('#').strip()` → `int()` 변환. fallback(`all_articles[:2]`) 자체를 제거하고 매칭 실패 시 스킵.

---

## 문제 5: 고유명사 음역 오류
**증상:** `Huawei` → `화웨웨이` (잘못된 음절 분절)
**원인:** GPT tokenization + 한국어 음역 과정에서 발생. pitcher hook(`엔비디아에 90억`)은 writer가 변경 불가.
**해결:** pitcher + writer 양쪽 시스템 프롬프트에 "고유명사는 영어 원문 사용" 규칙 추가.

---

## 문제 6: posted.json 저장 누락 → 중복 발행
**증상:** 같은 기사(예: 21658 GPT-5.5, 24700 Gemini)가 여러 번 발행됨
**원인:** `publisher.py`가 `articles[0]`(pool 첫 번째)만 posted.json에 저장. 피치가 실제 사용한 `article_ids`는 저장 안 됨.
**해결:** `publish_thread_chain` 호출 시 `articles[0]` 대신 `article_ids[0]`의 실제 기사 전달. 발행 후 피치의 **모든** article_ids와 links를 posted_ids/posted_links에 저장. 과거 미등록된 25개 기사도 일괄 등록.

---

## 문제 7: twist 키워드 검증 너무 빡빡함
**증상:** GPT-4o-mini fallback이 내용은 잘 생성했는데 `정도로`, `쏟아붓` 등의 잡음 키워드 때문에 발행 실패
**원인:** 추출된 키워드에 조사/부사 포함. 모델이 해당 형태를 정확히 포함시킬 확률 낮음.
**해결:** **twist 키워드 검증 자체를 제거.** hook 검증 + 연도 검증 + 카드 수 검증만 유지.

---

## 문제 8: 피치 이력 중복 → 발행 불가
**증상:** `"모든 피치가 이력과 중복"` — 7시간 동안 발행 0건
**원인:** pitch_history 30개가 dry-run/failed로 가득 참. 새로운 피치 생성 불가.
**해결:** 실발행 매칭 건만 유지하고 30→6개로 정리. `db_reader LIMIT 200→1000`, `max_articles 100→500`으로 풀 확장.

---

## 문제 9: Gemma-3n 프롬프트 누출
**증상:** 1번 카드에 `"핵심 이야기:", "반전:", "감정:", "체감 단위:"`가 그대로 출력됨
**원인:** Gemma-3n이 프롬프트 레이블을 문자 그대로 출력에 포함. DiffusionGemma는 이 문제 없음.
**해결:** DiffusionGemma로 복귀. 시스템 프롬프트에 피치 메타데이터 출력 금지 명시.

---

## 문제 10: 1번 카드 hook 단독 사용
**증상:** hook 한 줄만 1번 카드로 만들고 바로 `---`로 끊음 (5~6줄 필요)
**원인:** "첫 문장은 반드시 hook 그대로"를 극단적으로 해석
**해결:** 프롬프트에 "hook이 1번 카드의 유일한 문장이 되어서는 안 됨. 뒤에 내용을 이어붙여 5~6줄로 채워라." 명시.

---

## 문제 11: 부분 발행 (체인 중단)
**증상:** 6개 카드 중 3개만 발행되고 중단. 나머지는 수동 발행 필요.
**원인:** Threads API rate limit. 카드 간 3초 간격으로 3장 이후 rate limit 도달.
**해결:** 카드 간 대기 3→10초, 재시도 간격 2→10초. 체인 전체 ~60초로 안정화.

---

## 문제 12: DiffusionGemma 문장 축약
**증상:** `"해커들은 AI 쓰레기 데이터를 싫어하지만, 역설적으로 그 쓰레기가 그들의 무기를 만들고 있음."` — 한 줄 축약이 많음
**원인:** 프롬프트에 `"한 줄에 하나의 정보. 짧게 끊어서 리듬감"` 지시
**해결:** `"2~3줄로 서술. 한 줄짜리 축약 금지. 인과관계 설명. A→B가 아니라 A로 인해 B가 발생한 이유를 풀어써라."`로 변경.

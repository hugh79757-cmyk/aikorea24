...
**발행 프로세스:**
```
1. 각 카드 → add_line_spacing()으로 공백 정리
...
**add_line_spacing() (2026-06-23 → 2026-06-23 v2):**
- AI가 이미 stanza 구조(빈 줄 포함)로 작성했으면 **그대로 통과** (의도한 리듬 보존)
- 카드가 한 덩어리로 뭉쳐져 있을 때만 마침표/물음표/느낌표 기준으로 분리하여 `\n\n` 삽입
- 안전망 역할: AI가 리듬 규칙을 무시했을 때만 개입

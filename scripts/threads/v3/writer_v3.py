...
[카드 구조 — 5개, --- 로 구분]
1번 카드 (5~6줄): 첫 줄은 hook 그대로. 그 뒤에 구체적 사실(날짜/장소/숫자)을 이어붙여라. hook만 단독으로 쓰지 말고 5~6줄을 채워라. 사건의 출발점. 핵심 충돌을 설정한다. 독자가 "어?" 하고 멈추게 만든다.
...
- 축약 금지: "A → B" 한 줄 요약 대신 "A로 인해 B가 발생한 과정"을 최소 2~3문장으로 풀어써라.
...
- 첫 글자/숫자 드랍: "국 청소년"→"미국 청소년",  "년 만에"→"1년 만에",  "비디아"→"엔비디아",  "트로픽"→"앤트로픽"
- 단어 중간 글자 드랍: "운동하기 위한"→"운영하기 위한",  "모델 간 간"→"모델 간" (중복 글자 제거)
- 깨진/중복된 문자: "쏟아붓고"→"쏟아붓고",  "있는 있는"→"있는" (동일 단어 중복 제거)
- 잘못된 공백/특수문자: "'신발"→"신발",  "제조'"→"제조"

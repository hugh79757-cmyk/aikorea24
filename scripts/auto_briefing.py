...
def build_briefing(articles):
    ...
    return {"date": today, "intro": intro, "status": "draft", "items": items}
...
def save_briefing(data):
    ...
    existing = d1_query(f"SELECT id FROM briefings WHERE date = '{today}'")
    if existing:
        log(f"  오늘 브리핑({today}) 이미 존재 (id={existing[0]['id']})")
        return False

    intro_escaped = data["intro"].replace("'", "''")
    sql_briefing = (
        f"INSERT INTO briefings (date, intro, status, created_at) "
        f"VALUES ('{today}', '{intro_escaped}', 'draft', datetime('now'))"
    )
    if not d1_execute(sql_briefing):
        log("  브리핑 생성 실패")
        return False

    rows = d1_query(f"SELECT id FROM briefings WHERE date = '{today}'")
    if not rows:
        log("  브리핑 ID 조회 실패")
        return False
    briefing_id = rows[0]["id"]
    log(f"  브리핑 id={briefing_id} 생성")

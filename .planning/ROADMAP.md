...
**Status**: ✅ **Complete** (2026-08-12) — 5/5 plans 완료
**Mode**: mvp
**Depends on**: Phase 27
...
8. `generate_draft()`가 `a['link']`, `a['source']`, `a['published_at']`를 프롬프트에 포함
9. 원문 URL이 없는 기사는 자동 발행 차단 (초안 큐 보관)
10. 프롬프트에 원문 분류(flag 4개: has_numeric, has_comparison, has_source_entity, content_type) + 조건 분기 규칙 + 출처 강제 + content_type별 섹션 포함
11. 표 사용 금지 규칙 삭제, 조건부 표 필수 규칙 적용
12. 발행 전 자동 검수가 heuristic 4개(숫자 출처, 첫 120자 결론, 표 무결성, 제목 언어 일관성) + LLM 일반론 판정 하이브리드로 구현
13. `auto_deep_article.py` 프롬프트도 표·출처·독자행동·허브링크 포함하도록 수정
14. 표본 비교 rubric(6항목: 수치 출처 매칭률, 표 존재, 첫 120자 결론, 한국 독자 관점, 제목 한글 우선, 일반론 비율 ≤30%)으로 수정 전/후 평가 → 수정 후 평균 5/6 이상, 수정 전 대비 +3점 이상
15. 기존 275개 이상 테스트 통과
...
- 28-05: 글쓰기 로직 개선 — 원문 URL 입력 + 사실확인표 + 출처링크 + 독자행동 + 허브링크 + 발행 전 검수
